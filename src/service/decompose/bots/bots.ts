...
export async function handleNestedBotDecomposition(
  filePath: string,
  ...
  const recursivelyDisassembleBots = async (dir: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        await recursivelyDisassembleBots(fullPath);
      } else if (entry.isFile() && fullPath.endsWith('.xml')) {
        if (entry.name.includes('botDialogs-meta')) {
  ...
  if (existsSync(disassembledDir)) {
    await recursivelyDisassembleBots(disassembledDir);
  }
}
